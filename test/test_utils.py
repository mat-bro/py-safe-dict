...
def test_safe_func_on_dict(test_dict, info):
...
def test_safe_func_on_sequence(test_list_base):
    value = safe(test_list_base) >> [2] >> 'info' >> 'name'
    value2 = safe(test_list_base) >> [2] >> 'info' >= 'name'
...
def test_safe_func_on_dict_multi_index(test_dict, info):
